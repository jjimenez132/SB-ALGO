...
from data_pipeline.sources import prop_odds_player_props as source
...
def run_tests(base_date: dt.date) -> None:
    ...
    for label, target in (("Prop-Odds", yesterday), ("Prop-Odds", base_date)):
        data, errors = attempt_fetch(target)
        print_section(label, target, data, errors)


def main() -> None:
    parser = argparse.ArgumentParser(description="Prop-Odds NBA player props harness")
    parser.add_argument(
        "--today",
