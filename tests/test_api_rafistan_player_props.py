...
from data_pipeline.sources import rafistan_player_props as source
...
def run_tests(base_date: dt.date) -> None:
    ...
    for target in (yesterday, base_date):
        ...
        print_section("Rafistan RapidAPI", target, data, errors)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="NBA Player Props Odds (Rafistan RapidAPI) test harness"
    )
    parser.add_argument(
        "--today",
