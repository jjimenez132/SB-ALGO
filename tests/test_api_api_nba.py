...
from data_pipeline.sources import api_nba as source

APIError = source.APINBAError
PLAYER_ID = 15  # Giannis Antetokounmpo
TEAM_ID = 17  # Milwaukee Bucks
GAME_ID = 12345
...
def run_tests(base_date: dt.date) -> None:
    ...
    player_stats, errors = attempt_fetch(source.fetch_player_stats, PLAYER_ID, season=2023)
    print_section("Player Stats", player_stats, errors)

    team_stats, errors = attempt_fetch(source.fetch_team_stats, TEAM_ID, season=2023)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="API-NBA RapidAPI test harness")
    parser.add_argument(
        "--today",
